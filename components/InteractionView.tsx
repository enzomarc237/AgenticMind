...
const EntryIcon: React.FC<{ type: LogEntry['type'] }> = ({ type }) => {
    const commonClasses = "w-6 h-6 text-gray-500 dark:text-gray-400";
    switch(type) {
...
const CodeBlock: React.FC<any> = ({ node, inline, className, children, ...props }) => {
    ...
    if (inline) {
        return <code className="bg-gray-200 dark:bg-gray-700 text-purple-600 dark:text-purple-300 px-1 py-0.5 rounded text-sm" {...props}>{children}</code>;
    }
    ...
    const codeContent = match ? (
        ...
        <pre className="bg-gray-100 dark:bg-gray-800 p-4 rounded-md overflow-x-auto"><code className="text-gray-800 dark:text-white" {...props}>{children}</code></pre>
    );
    ...
    return (
        <div className="relative my-2 text-sm">
             <button 
                ...
                className={`absolute top-2 right-2 z-10 bg-gray-300 dark:bg-gray-700 hover:bg-gray-400 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-2 py-1 text-xs rounded transition-all
                    ${isCopied ? 'bg-green-600 text-white dark:text-white' : ''}
                `}
...
const markdownComponents: React.ComponentProps<typeof ReactMarkdown>['components'] = {
    h1: ({node, ...props}) => <h1 className="text-2xl font-bold my-4 border-b border-gray-200 dark:border-gray-700 pb-2" {...props} />,
    h2: ({node, ...props}) => <h2 className="text-xl font-bold my-3 border-b border-gray-200 dark:border-gray-700 pb-1" {...props} />,
    ...
    th: ({node, ...props}) => <th className="border border-gray-300 dark:border-gray-600 px-4 py-2 text-left" {...props} />,
...
const EntryCard: React.FC<{ children: React.ReactNode; entry: LogEntry; }> = ({ children, entry }) => {
  const bgColor = entry.type === 'user' ? 'bg-gray-100 dark:bg-gray-800' : 'bg-transparent dark:bg-gray-800/50';
  const borderColor = entry.type === 'error' ? 'border-red-500/50' : 'border-gray-200 dark:border-gray-700';
  ...
  return (
    <div className={`flex items-start space-x-4 p-4 rounded-lg border ${bgColor} ${borderColor}`}>
      ...
      <div className="flex-1 text-gray-700 dark:text-gray-300 space-y-2 min-w-0">{children}</div>
    </div>
  );
};
        ...
        <div className="space-y-4 max-w-4xl mx-auto">
          ...
          {isLoading && (
            <div className="flex items-center space-x-3 p-4">
              <LoadingSpinner className="w-6 h-6 text-purple-500 dark:text-purple-400" />
              <span className="text-gray-500 dark:text-gray-400 animate-pulse">Agent is thinking...</span>
            </div>
          )}
